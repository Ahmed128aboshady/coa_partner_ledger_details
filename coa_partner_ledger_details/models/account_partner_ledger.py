...
DETAIL_EXPAND_FN = '_report_expand_unfoldable_line_partner_ledger_coa_details'
...
class PartnerLedgerCustomHandler(models.AbstractModel):
    ...
    def _report_expand_unfoldable_line_partner_ledger(
        ...
        for line in result.get('lines', []):
            try:
                self._coa_tag_invoice_line(report, line, options)
            except Exception:
                _logger.exception(
                    "COA partner-ledger: failed to tag line %s",
                    line.get('id'))
        return result

    def _coa_tag_invoice_line(self, report, line, options):
    ...
    # ------------------------------------------------------------------
    def _report_expand_unfoldable_line_partner_ledger_coa_details(
            self, line_dict_id, groupby, options, progress, offset,
            unfold_all_batch_data=None):
        report = self.env['account.report'].browse(options['report_id'])
        ...
        if aml:
            ...
            for pline in product_lines:
                lines.append(self._coa_build_product_line(
    ...
    # ------------------------------------------------------------------
    def _coa_build_product_line(self, report, options, parent_line_id,
                                pline, level, aml):
        """
        ...
        line_id = report._get_generic_line_id(
            ...
            parent_line_id=parent_line_id, markup='coa_detail')

        # ── Label ────────────────────────────────────────────────────────
        qty_txt = self._coa_fmt_qty(pline.quantity)
        ...
        for col in options.get('columns', []):
            ...
            if expr == 'debit' and is_debit_side:
                columns.append({
                    'name': self._coa_fmt(report, options, amount, currency),
                    'no_format': amount,
                    'class': 'number',
    ...
    # Helpers
    # ------------------------------------------------------------------
    def _coa_fmt(self, report, options, value, currency):
        try:
            ...
            try:
                return report.format_value(
                    value, currency=currency, figure_type='monetary')
            except Exception:
                return str(value)

    def _coa_fmt_qty(self, qty):
        if qty == int(qty):
            return str(int(qty))
